...
class ReportController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json(['data' => [
            ...
            'sales_by_rep' => $dealDate(Deal::query())->leftJoin('users', 'users.id', '=', 'deals.assigned_to')->where('deals.status', 'won')->selectRaw("COALESCE(users.name, 'Unassigned') label, COUNT(*) total, SUM(deals.value) value")->groupBy('users.name')->orderByDesc('value')->get(),
            'lead_aging' => $leadDate(Lead::query())->selectRaw("CASE WHEN DATEDIFF(NOW(), leads.created_at) <= 7 THEN '0–7 days' WHEN DATEDIFF(NOW(), leads.created_at) <= 30 THEN '8–30 days' WHEN DATEDIFF(NOW(), leads.created_at) <= 90 THEN '31–90 days' ELSE '90+ days' END label, COUNT(*) total")->groupBy('label')->orderByRaw('MIN(leads.created_at)')->get(),
